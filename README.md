...
**Phase 1C graph quality:** [`UPG-GOV-TMG-EKG-GRAPH-QUALITY-PHASE1C-001`](https://github.com/TMGContracting/governance/blob/main/UPG/UPG-GOV-TMG-EKG-GRAPH-QUALITY-PHASE1C-001.md) — use **`--emit-version v0_1`** for deterministic enrichment (R4 rule pack; evidence **`rule_pack_version`:** `UPG-GOV-TMG-EKG-GRAPH-QUALITY-PHASE1C-001-R4`). Default **`v0`** remains prototype **v0** output.
...
# Phase 1C (v0.1 + enrichment): add --emit-version v0_1
...
This writes `data/index_runs/<RUN_ID>/graph.json` and `data/index_runs/<RUN_ID>/evidence.json` in this repository only, validates them against **v0** (default) or **v0.1** (`--emit-version v0_1`) schemas, and **does not** open network connections, modify the input working tree, or use mutating `git` subcommands. Read-only `git` in the **input** root (for `origin` URL) and in the **implementation** repo (for `repo_commit` in evidence) is allowed. For the evidence `outputs[1].content_sha256` field, the prototype uses the 64-ASCII-`0` **template** hash (see `docs/feasibility/PHASE1_FEASIBILITY_PACKET.md` context).
